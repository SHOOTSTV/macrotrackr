...
import { deleteMealById, patchMeal } from "@/src/lib/services/meals";
...
export async function DELETE(
  ...
  const ip = getClientIp(request);
  if (!(await checkRateLimit(`delete:meal:${ip}`))) {
    return tooManyRequests();
  }
  ...
  if (request.headers.get("x-dashboard-ui") !== "1") {
    return Response.json(
      {
        error: "forbidden",
        message: "Meal deletion is only allowed from dashboard UI",
      },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    await deleteMealById(routeParams.data.id, userId);
